...
- **Per-Preset Formatting Constraints / Client-Side Multi-Mode Rendering**: Output formatting constraints (plain text, Markdown, or specialized formats) are now defined **per-preset** rather than enforced globally via the fallback `system_prompt.txt` (which was emptied in v2.4.0). On the client side, the `GenerationResultView` component renders output in three user-selectable modes: **Formatted Markdown** (via `react-markdown` with styled component overrides), **Raw Monospace** (traditional `pre`/`code` block), and **Syntax-Highlighted JSON** (with per-token color coding, line numbers, and auto-lock behavior when structured output is enabled), with a toggle displayed in the output panel footer.
...
- **Agentic Video Understanding**: PromptLab natively supports Google's **Agentic Video Understanding** (`mediaProcessing: "AGENTIC"`) for Gemini 3.7 Flash models. Users can toggle individual video assets between `STATIC` (standard 1 FPS fixed-rate frame sampling, default) and `AGENTIC` (dynamic timeline navigation using model-driven tool calls) directly from the `VideoAssetCard`. YouTube video references passed as `fileData` include explicit `mimeType: "video/mp4"`. When an unsupported model (such as Gemini 2.5 Pro) is active, the video card preserves the user's `AGENTIC` selection in memory, displays a dashed amber `Paused` status badge, and automatically executes as `STATIC` fallback on the backend until switching back to Gemini 3.7 Flash.
...
- **Client-side multi-mode rendering**: The `GenerationResultView` component (`components/GenerationResultView.tsx`) renders output in three user-selectable modes:
...
- The view mode toggle is displayed in the footer of the generation output panel as `VIEW: FORMATTED MARKDOWN` / `VIEW: RAW MONOSPACE` / `VIEW: JSON VIEW` (or `VIEW: STRUCTURED JSON (LOCKED)` when engine-level structured output is active).
