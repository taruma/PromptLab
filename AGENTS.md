...
Key differentiators:
- **Zero Database / Hybrid Persistence**: Uses static filesystem templates for fallback config defaults, client-side `localStorage` for active session configurations, and local presets for quick workspace instantiation. Project data, configurations, and image blobs are persisted in **IndexedDB** (`promptlab_db` v2) to avoid browser localStorage quota limits, with automatic synchronization to legacy `localStorage` keys for backward compatibility.
...
- **Client-Side Storage**: **IndexedDB** (`promptlab_db` v2 → `images` and `projects` object stores) for image blob and project data persistence, avoiding browser localStorage quota constraints.
