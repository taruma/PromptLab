...
- **Project Import/Export**: `exportProjectJSON()` packages the full project configuration plus all asset image blobs (retrieved from IndexedDB `images` store) into a downloadable JSON blob. `importProjectJSON()` validates the payload, restores images to IndexedDB, handles name collisions with incrementing suffixes, and saves the imported project. Exported filenames use the format `PromptLab-Project-{slugified_name}_{date}_{time}_{uniqueId}.json`.
