...
- **Session History Persistence**: Generation results are automatically archived to localStorage (`prompt_generator_history_v1`), preserving the generated text, active variable states, image references, video metadata, and the current system prompt and prompt template for future recall. History items store `systemPrompt` and `promptTemplate` for archival, export, and diff comparison purposes.
