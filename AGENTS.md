...
- **Strict Server-Side Constraints / Client-Side Dual-Mode Rendering**: The system prompt strictly mandates that the model output be formatted as **pure, standard plain text** with zero Markdown symbols at generation time. On the client side, the `GenerationResultView` component renders output in two user-selectable modes: **Formatted Markdown** (via `react-markdown` with styled component overrides) and **Raw Monospace** (traditional `pre`/`code` block), with a toggle displayed in the output panel footer.
