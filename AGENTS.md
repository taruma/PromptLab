...
- **Video History Metadata**: Video references are persisted in history items via an optional `videos` array storing label, mimeType, and duration metadata (Base64 blobs are not stored due to size constraints). History recall restores video metadata to the workspace; full video re-upload is required for regeneration. History import/export preserves video references with the `HistoryVideoRef` interface in `lib/history-export.ts`.
