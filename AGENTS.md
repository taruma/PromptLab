...
- **Image Blob Storage**: Raw Base64 image data is stored in IndexedDB (`promptlab_db` v2, `images` object store with `id` keyPath) rather than localStorage, avoiding browser storage quota limits (~5-10MB for localStorage vs hundreds of MB for IndexedDB).
...
- **Project-Based Data Model**: Each workspace (project) is stored as a `Project` object in IndexedDB (`promptlab_db` v2 `projects` store) with its own `systemPrompt`, `promptTemplate`, `customPresets` array, `history` array, and `assetLibrary` array. The `Project` interface is defined in `lib/projects.ts` alongside CRUD helpers (`getAllProjects`, `getProject`, `saveProject`, `deleteProject`).
