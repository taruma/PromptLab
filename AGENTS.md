...
- **Per-Preset Formatting Constraints / Client-Side Multi-Mode Rendering**: Output formatting constraints (plain text, Markdown, or specialized formats) are now defined **per-preset** rather than enforced globally via the fallback `system_prompt.txt` (which was emptied in v2.4.0). On the client side, the `GenerationResultView` component renders output in four user-selectable modes: **Formatted Markdown** (via `react-markdown` with styled component overrides), **Raw Monospace** (traditional `pre`/`code` block), **Syntax-Highlighted JSON** (with per-token color coding, line numbers, and auto-lock behavior when structured output is enabled), and **Auteur Script View** (structured macro-state / sub-state rendering with persistent collapsible staging blocks, compact execution styling, and smart auto-detection), with a toggle displayed in the output panel footer.
...
- **Client-side multi-mode rendering**: The `GenerationResultView` component (`components/GenerationResultView.tsx`) renders output in four user-selectable modes:
...
- The view mode toggle is displayed in the footer of the generation output panel as `VIEW: FORMATTED MARKDOWN` / `VIEW: RAW MONOSPACE` / `VIEW: JSON VIEW` / `VIEW: AUTEUR VIEW` (or `VIEW: STRUCTURED JSON (LOCKED)` when engine-level structured output is active).
...
4. **Tri-Surface Parity**:
   - The Auteur Script rendering modality must be supported identically across all three generation inspection surfaces:
