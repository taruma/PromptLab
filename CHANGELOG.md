...
- **User Presets Import/Export System Hardening & Enhancement (`lib/preset-export.ts`, `components/PresetImportConfirmModal.tsx`, `components/PresetExportDropdown.tsx`, `components/PresetReplaceConfirmModal.tsx`, `hooks/use-url-preset-import.ts`).**
  - **Defensive Type Validation & Foreign Package Rejection**: Added strict type checks in `lib/preset-export.ts` (`readAndValidatePresetsJSON` and `importPresetsFromJSON`). Accidental uploads of History exports (`promptlab_history_export`), Asset Library backups (`promptlab_asset_library`), or Project backups (`promptlab_project`) are explicitly rejected with descriptive guidance pointing users to the appropriate explorer or modal.
  - **Heuristic History Collision Prevention**: Hardened preset item extraction against untyped history files with `items: [...]` by inspecting properties (`generationResult`, `outputs`, `specs`), preventing generation records from inadvertently polluting the user preset library.
  - **100% Backward-Compatible Multi-Format Parser**: Seamlessly parses modern versioned packages (`promptlab_presets_export` with `presets`), legacy containers (`items`), raw JSON arrays of `UserPreset[]`, single preset wrappers (`{ preset: { ... } }`), and single raw preset objects (`{ name, systemPrompt, promptTemplate }`).
  - **Architectural Parity (`readAndValidatePresetsJSON`)**: Introduced a standardized asynchronous file validation function returning `{ success, data, rawItems, error }`, matching the contracts of `readAndValidateProjectJSON` and `readAndValidateAssetLibraryJSON`.
  - **Modal Tier Alignment (`PresetImportConfirmModal.tsx`, `PresetReplaceConfirmModal.tsx`)**: Realigned the confirmation dialog and loading overlay from generic `z-50` to standardized **Tier 3 `z-[60]`**, elevated error/success toasts to **Tier 4 `z-[70]`**, and registered with the universal LIFO Escape key stack (`useModalEscape`), ensuring sub-modals dismiss cleanly above `PromptConfigModal` (`z-50`).
  - **Capture-Phase Escape Isolation (`PresetExportDropdown.tsx`)**: Elevated the export dropdown menu to **Tier 4 `z-[70]`** and attached a document capture-phase Escape keydown listener (`e.stopImmediatePropagation()`), guaranteeing that pressing Escape closes only the dropdown without closing the parent `PromptConfigModal`.

- **Project Workspace Backup & Restore System Architecture Overhaul (`lib/projects.ts`, `components/ProjectManagerModal.tsx`, `components/ProjectImportConfirmModal.tsx`, `components/ProjectExportModal.tsx`).**
  - **Configurable Export Scope Modal (`ProjectExportModal.tsx`)**: Introduced a dedicated Tier 3 (`z-[60]`) export dialog with LIFO Escape dismissal (`useModalEscape`). Provides 3 quick-scope presets: **Full Backup** (complete archive with prompts, presets, assets, and full generation history with reference images), **Compact History** (includes all prompts, presets, assets, and history text/metrics, but strips heavy history image blobs to keep file size lightweight), and **Starter Template** (prompts, presets, and assets only; zero history records for clean project sharing). Includes fine-grained component checkboxes and a real-time package breakdown ribbon.
  - **Critical History Image Preservation Fix**: Resolved an issue where `exportProjectJSON` previously only extracted images from `project.assetLibrary`, omitting reference image blobs attached to `project.history` entries. Now both `assetLibrary` and `project.history` images are aggregated into a unified pool, preventing image loss when backing up and restoring workspaces.
  - **v1.1 Deduplicated Image Pool**: Re-architected project JSON exports to store unique image Base64 payloads once in a top-level `images: Record<string, string>` dictionary keyed by SHA-256 `contentHash` (or fallback ID), while stripping inline Base64 payloads from individual history items (`img.base64 = ""`). This cuts backup file sizes dramatically and maintains 100% backward compatibility with legacy v1.0 array-based exports (`{ id, base64 }[]`).
  - **Chunked Streaming Blob Serialization**: Replaced monolithic JSON serialization with discrete string chunks streamed directly into `new Blob(chunks, { type: "application/json" })`, eliminating V8 single-string heap allocation limits (`RangeError: Invalid string length`) when exporting workspaces with large multi-modal assets.
  - **Pre-Import Workspace Inspection Modal (`ProjectImportConfirmModal.tsx`)**: Introduced a dedicated Analog Brutalist dialog (Tier 3 `z-[60]`) with LIFO Escape key dismissal (`useModalEscape`). Features a real-time summary breakdown ribbon (`PRESETS`, `HISTORY`, `ASSETS`, `IMAGES`), project name collision resolution with auto-suffixing, optional description editing, tabbed prompt inspection (System Prompt vs. Prompt Template), and an auto-switch toggle.
  - **100% Backward Compatibility & Unified Hydration**: Seamlessly accepts both modern v1.1 image pools and legacy v1.0 image arrays. During import, automatically pre-hydrates IndexedDB `images` store with event-loop yielding (`setTimeout(0)` batches of 10) to keep the browser UI responsive, and cleans up legacy inline Base64 from historical records to prevent IndexedDB `projects` store bloat.
  - **Defensive Format Validation**: Added strict file-type checking in `readAndValidateProjectJSON`, catching and rejecting accidental uploads of History exports (`promptlab_history_export`), Asset Library backups (`promptlab_asset_library`), or User Presets (`promptlab_user_presets`) with actionable error messages.

- **Asset Library Import/Export System Architecture Overhaul (`lib/asset-library-export.ts`, `components/AssetExportDropdown.tsx`, `components/AssetImportModal.tsx`, `components/AssetLibrarySidebar.tsx`, `lib/projects.ts`).**
  - **v1.1 Deduplicated Image Pool**: Extracted Base64 images into a top-level shared `images: Record<string, string>` pool indexed by SHA-256 `contentHash`, eliminating multi-megabyte Base64 payload duplication and maintaining 100% backward compatibility with legacy v1.0 imports.
  - **Chunked Streaming Blob Serialization**: Replaced monolithic `JSON.stringify(..., null, 2)` with discrete array chunks streamed directly into `new Blob(chunks, { type: "application/json" })`, bypassing the V8 engine single-string memory ceiling (~512MB) and eliminating `RangeError: Invalid string length` crashes on large image asset libraries.
  - **Pre-Import Visual Inspection Modal (`AssetImportModal.tsx`)**: Completely redesigned the asset import dialog into an Analog Brutalist inspection dashboard featuring a real-time summary breakdown ribbon (`TOTAL IN FILE`, `+NEW UNIQUE`, `ALREADY EXISTS`), scrollable thumbnail preview grid with `NEW` (emerald) and `EXISTS` (amber) status tags, and intelligent strategy controls (`Merge` vs `Overwrite`).
  - **Dual-Criteria Duplicate Detection**: Implemented dual-matching by **SHA-256 `contentHash`** OR **asset `id`**, coupled with an asynchronous background hash resolver for existing session assets that lack pre-computed hashes. Guarantees 100% accurate duplicate detection on re-imports of existing library exports.
  - **Export Favorites / Pinned**: Added an **"Export Favorites"** action to `AssetExportDropdown.tsx` with live starred/pinned count badge and capture-phase Escape dismissal per the project modal/overlay architecture.
  - **Defensive Format Validation**: Integrated explicit validation on `parsed.type`, rejecting accidental uploads of Project backups (`promptlab_project`), History exports (`promptlab_history_export`), or Presets exports (`promptlab_user_presets`) with helpful error banners.
  - **Persistent Hashing Across Lifecycle**: Added `contentHash` to `LibraryImage` and `ProjectAsset` interfaces, ensuring hashes are computed on upload, hydrated from IndexedDB on load, and retained when syncing to localStorage.

- **Output Renderer Modal & Shared Multi-Mode Output Architecture (`components/OutputRendererModal.tsx`, `components/MultiModeOutputView.tsx`, `lib/output-render-helpers.tsx`, `components/AppHeader.tsx`, `app/page.tsx`).**
  - **Header Navigation Trigger (`AppHeader.tsx`)**: Added a top navigation button (`Renderer`, `#output-renderer-header-btn`) styled with a retro cyan accent (`bg-cyan-50/80 text-cyan-950 border-cyan-300/80`) and `Eye` icon to open the Output Renderer modal from anywhere in the workspace.
  - **Distraction-Free Split-Screen Modal (`OutputRendererModal.tsx`)**: Built a full-bleed split workspace dialog (Tier 2 `z-50`) adhering to `.agents/rules/modal-overlay-architecture.md` and registered with the universal LIFO Escape key stack (`useModalEscape`) and compound safety gate (`isAnyModalActive`). The left pane provides an edge-to-edge source editor with live line and character counters, while the right pane provides an instant live preview.
